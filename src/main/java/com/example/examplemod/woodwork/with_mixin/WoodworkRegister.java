...
public record WoodworkRegister(Woodwork woodwork) {
    ...
    @Nullable
    private static Method methodGenerateRecipes = null;
    ...
    public void registerTileEntityModifiers() {
        if (!woodwork.hasCustomSignEntity()) {
            ((IBlockEntityType) BlockEntityType.SIGN).addBlockToEntity(woodwork.sign(), woodwork.wallSign());
        }
        if (woodwork.hasChest() && !woodwork.hasCustomChestEntity()) {
            ((IBlockEntityType) BlockEntityType.CHEST).addBlockToEntity(woodwork.chest());
        }
    }
    ...
    public record WoodworkRegisterEvents(WoodworkRegister register) {

        public void onSetup(FMLCommonSetupEvent event) {
            register.registerTileEntityModifiers();
        }

        public void onClient(FMLClientSetupEvent event) {
            register.registerRenderType();
        }
        ...
        public Object client() {
            return new Object() {
                ...
                public void onLayerDefinition(EntityRenderersEvent.RegisterLayerDefinitions event) {
                    register.registerBoatLayer(event);
                }
            };
        }
    }
    ...
    public void addRecipes(Consumer<FinishedRecipe> consumer, @Nullable Block log) {
        ...
        if (methodGenerateRecipes == null) {
            methodGenerateRecipes = Arrays.stream(RecipeProvider.class.getDeclaredMethods())
                    .filter(m -> Modifier.isStatic(m.getModifiers()))
                    .filter(m -> m.getParameterCount() == 2)
                    .filter(m -> m.getReturnType() == Void.TYPE)
                    .filter(m -> m.getParameterTypes()[0] == Consumer.class && m.getParameterTypes()[1] == BlockFamily.class)
                    .findFirst()
                    .orElseThrow();
            methodGenerateRecipes.setAccessible(true);
        }
        Objects.requireNonNull(methodGenerateRecipes);
        try {
            methodGenerateRecipes.invoke(null, consumer, new BlockFamily.Builder(woodwork.planks())
